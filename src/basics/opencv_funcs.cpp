...
// ---------------- image operation ----------------
vector<unsigned char> getPixelAt(
...
unsigned char getPixelAt(
    const cv::Mat &image, int row, int col, int idx_rgb) {
...
double calcAngleBetweenTwoVectors(
    const cv::Mat &vec1, const cv::Mat &vec2) {
